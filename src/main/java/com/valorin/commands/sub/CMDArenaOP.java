...
public class CMDArenaOP extends SubCommand implements AdminCommand {
    ...
    public void sendHelp(Player p) {
        ...
        sm("&b/dt arena(a) setkit <竞技场编辑名> &f- &a为某个竞技场设置开局给予玩家的固定装备(KitPVP模式)", p, false);
        sm("&b/dt arena(a) enablekit <竞技场编辑名> &f- &a设置好固定装备后，输出该指令以将某个竞技场设置为KitPVP模式", p, false);
        sm("&b/dt arena(a) disablekit <竞技场编辑名> &f- &a取消某个竞技场的KitPVP模式", p, false);
        sm("", p);
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label,
        ...
        if (args[1].equalsIgnoreCase("setkit")) {
            ...
            sm("&a[v]已打开面板，请将装备放到里面", p);
            return true;
        }
        if (args[1].equalsIgnoreCase("enablekit")) {
            if (args.length != 3) {
                sm("&7正确用法：/dt a enablekit <竞技场编辑名>", p);
                return true;
            }
            String editName = args[2];
            if (!Data.getArenas().contains(editName)) {
            ...
            sm("&a[v]KitPVP模式开启成功！在这个竞技场比赛的玩家会在比赛开始后临时使用你设定的Kit装备", p);
            return true;
        }
        if (args[1].equalsIgnoreCase("disablekit")) {
            if (args.length != 3) {
                sm("&7正确用法：/dt a disablekit <竞技场编辑名>", p);
                return true;
            }
            String editName = args[2];
            if (!Data.getArenas().contains(editName)) {
