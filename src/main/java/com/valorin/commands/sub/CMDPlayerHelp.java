...
public class CMDPlayerHelp extends SubCommand {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label,
		...
		sm("&b/dt timetable(tt) &f- &a查看匹配功能开放时间段", p, false);
		sm("&b/dt arenasinfo(ainfo) &f- &a查看所有竞技场信息", p, false);
		sm("&b/dt watch(w) <竞技场名称> &f- &a观战某个正在比赛的竞技场", p, false);
		sm("&b/dt send <玩家名> [竞技场名称(不填则随机)] &f- &a向某个玩家发送单挑请求", p, false);
		sm("&b/dt accept &f- &a接受某个玩家的单挑请求", p, false);
		sm("&b/dt deny &f- &a拒绝某个玩家的单挑请求", p, false);
		sm("&b/dt sendall(sa) &f- &a向全服发起单挑请求(需要特定邀请函)", p, false);
		...
		sm("&b/dt changelang(cl) &f- &a查看所有语言文件", p, false);
		sm("&b/dt changelang(cl) <语言文件> &f- &e切换语言", p, false);
		sm("", p);
		return true;
	}
